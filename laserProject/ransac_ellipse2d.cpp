...
namespace sac
{
    ...
    double cvmGet(const Mat& m, int row, int col) {
	...
	bool ransacModelEllipse2D::isGoodSample(const std::vector<int> &samples) const
	{
		...
		Mat A = Mat(5, 6, CV_64F, aa);
        Mat D;  D.create(5, 6, CV_64F);
        Mat U;  U.create(5, 5, CV_64F);
        Mat V;  V.create(6, 6, CV_64F);
        SVDecomp(A, D, U, V,SVD::FULL_UV);
		...
		Mat ellParamMat = Mat(3, 3, CV_64F, ellParam);		

		double thres2 = determinant(ellParamMat)*(epA + epC);
